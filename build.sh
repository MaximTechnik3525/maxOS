...
$CC -c src/kernel/task.c -o build/task.o $CFLAGS
$CC -c src/drivers/ata.c -o build/ata.o $CFLAGS
$CC -c src/drivers/pci.c -o build/pci.o $CFLAGS
$CC -c src/drivers/ahci.c -o build/ahci.o $CFLAGS
...
$CC -c src/drivers/video.c -o build/video.o $CFLAGS
...
$LD --no-warn-rwx-segments -T src/linker.ld -o build/mykernel.bin \
    ...
    build/string.o build/dialog.o build/debug.o build/verbose_boot.o build/video.o build/mmu.o build/pmm.o build/malloc.o build/elf.o build/idt.o build/user.o build/task.o build/kernel.o build/ata.o build/pci.o build/ahci.o build/libc.o \
...
GRUB_DEFAULT=0
GRUB_TIMEOUT=3
if [ "$1" == "--verbose" ] || [ "$1" == "--uefi-verbose" ]; then
    GRUB_DEFAULT=1
fi
if [ "$1" == "--uefi" ] || [ "$1" == "--uefi-verbose" ]; then
    GRUB_TIMEOUT=1
fi

# Генерируем конфигурационный файл GRUB с поддержкой BIOS и UEFI GOP
...
echo " Совместимость: Dual-Boot (Legacy BIOS + UEFI GOP)"
...
if [ "$1" == "--boot-hdd" ]; then
    ...
    echo " Запуск maxOS в режиме UEFI (OVMF) x86_64..."
    qemu-system-x86_64 -bios /usr/share/ovmf/OVMF.fd -m 512M -audiodev alsa,id=snd0 -machine pcspk-audiodev=snd0 -drive file=maxos_disk.img,format=raw,index=0,media=disk -cdrom maxos.iso -boot d -serial stdio
elif [ "$1" == "--uefi-verbose" ]; then
    echo " Запуск maxOS в режиме UEFI (OVMF) x86_64 с подробной загрузкой..."
    qemu-system-x86_64 -bios /usr/share/ovmf/OVMF.fd -m 512M -audiodev alsa,id=snd0 -machine pcspk-audiodev=snd0 -drive file=maxos_disk.img,format=raw,index=0,media=disk -cdrom maxos.iso -boot d -serial stdio
elif [ "$1" == "--verbose" ]; then
    ...
    echo " Запуск в QEMU x86_64 с подключенным жестким диском (BIOS)..."
    qemu-system-x86_64 -audiodev alsa,id=snd0 -machine pcspk-audiodev=snd0 -drive file=maxos_disk.img,format=raw,index=0,media=disk -cdrom maxos.iso -boot d -serial stdio
fi